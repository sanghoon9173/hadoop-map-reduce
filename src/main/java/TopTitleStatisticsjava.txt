...
// <<< Don't Change
        ...
        public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
        	...
        	StringTokenizer tokenizer = new StringTokenizer(value.toString(), this.delimiters);
        	 while (tokenizer.hasMoreTokens()) {
                 ...
                 String currentString = nextToken.trim().toLowerCase();
                 if (!stopWords.contains(currentString)) {
                     context.write(new Text(currentString), new IntWritable(1));
                 }
             }
        }
    }
    ...
    public static class TitleCountReduce extends Reducer<Text, IntWritable, Text, IntWritable> {
        ...
        public void reduce(Text key, Iterable<IntWritable> values, Context context) throws IOException, InterruptedException {
            //counter for summing up the total count on each word
        	int totalCount =0;
            for(IntWritable word:values) {
            	//append the count that was incremented from mapper
            	totalCount+=word.get();
            }
    ...
    public static class TopTitlesStatReduce extends Reducer<NullWritable, TextArrayWritable, Text, IntWritable> {
        ...
        public void reduce(NullWritable key, Iterable<TextArrayWritable> values, Context context) throws IOException, InterruptedException {
            ...
            //calculate variance
            for(int i=0; i< N; i++) {
            	var += (int) (Math.pow(topCounts[i]-mean, 2));
            }
            
            var = (int) Math.floor(var/N);
